...
final class TripScheduleSnapshotEncoderTests: XCTestCase {
    ...
    func test_scheduleItems_preservesExplicitAirlinePrefix() {
        let leg = makeLeg(
            flight: "XX001",
            depAirport: "ANC",
            arrAirport: "CVG",
        ...
        let payload = TripScheduleSnapshotEncoder.encode(
            ownerDisplayName: "0000001",
            schedules: [makeSchedule(legs: [leg])],
            now: iso("2026-06-01T00:00:00Z")
        )

        XCTAssertEqual(payload.scheduleItems[0].label, "XX001 ANC-CVG")
    }
    ...
    func test_crewAccessTrips_formatsDeadheadAndXXFlightText() {
        let payload = TripScheduleSnapshotEncoder.encode(
            ownerDisplayName: "7793942",
            crewAccessTrips: [
                makeCrewAccessTrip(
