...
- **D-7 (ActivityKit/SpringBoard runtime acceptance):** `docs/DEVICE_VERIFICATION_CHECKLIST.md` verifies that Lock Screen and Dynamic Island expanded render numeric durations without dash/blank redaction. Later PO approval accepts the recorded production-path Simulator runtime matrix; test-host rendering alone remains insufficient.
