...
TMP_SWIFT="$(mktemp /tmp/crewaccess_confirm_txn.XXXXXX.swift)"
TMP_BIN="$(mktemp /tmp/crewaccess_confirm_txn.XXXXXX.bin)"
...
struct Main {
    static func main() async throws {
        guard CommandLine.arguments.count >= 2 else {
            fputs("usage: validate_confirm_transaction <repo_root>\n", stderr)
            Foundation.exit(2)
        }
        ...
        let cacheNotSaved = (cache.saveCount == 0)
        let hasFailureMessage = message.contains("Import failed") && message.contains("No changes were applied")

        if schedulesUnchanged && cacheNotSaved && pendingStillThere && hasFailureMessage {
            print("PASS validate_confirm_transaction")
            ...
            fputs("FAIL validate_confirm_transaction\n", stderr)
